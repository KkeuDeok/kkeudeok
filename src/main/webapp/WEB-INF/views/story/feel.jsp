...
<% if (wrong != null) {
       ...
       String trait = "", clue = "";
       for (String[] r : wrongTraits) { if (r[0].equals(wrong)) trait = r[1]; }
       for (String[] r : answerClues) { if (r[0].equals(emo))   clue  = r[1]; }
%>
<div class="story-dim" role="dialog" aria-modal="true" aria-labelledby="wrongTitle">
    <div class="story-modal">
        <div class="hd">
            <h2 id="wrongTitle">토리의 귓속말</h2>
            <%-- 닫아도 다시 고를 수 있게 모달 없는 같은 화면으로 돌아간다 --%>
            <a class="x" href="/story/feel?emo=<%= emo %>" aria-label="닫기"></a>
        </div>
        <div class="bd">
            <%-- Figma 는 `캐릭터 - 슬픔` 고정. 정답 감정별 그림은 자산이 없다 --%>
            <img src="/img/story-tori-sad.png?v=142" alt="">
            <p class="say">"음... 내 마음은 조금 다른 것 같아"</p>
            <p class="clue"><%= trait %> <%= clue %></p>
            <p class="more">가까워지고 있어! 조금만 더 살펴보자.</p>
        </div>
        <div class="ft">
            <a href="/story/feel?emo=<%= emo %>">다시 볼래!</a>
        </div>
    </div>
</div>
<% } %>
